...
# histogram setting
deltav = 4.*L1/100 # slotwidth for v histogram
vdist = graph(x=800, y=0, ymax = ymax,width=500, height=300, xtitle='x', ytitle='density(N)', align = 'left')
...
freqSlider = slider(min=0, max=15000, value=freq, bind=setFrequency)
 
def setAmplitude(amplitude):
  amplitude = amplitudeSlider.value
 
scene.append_to_caption('\n\nPiston amplitude\n')    
amplitudeSlider = slider(min=0, max=L1, value=0.005, bind=setAmplitude)
...
freqText = label(text = 'Freq: ' + str(freq) + " Hz", pos = vec(L1,L1, 0), box = False, align = 'left' )
ampText = label(text = 'Amplitude: ' + str(amplitude*100) + ' cm', pos = vec(L1,  -L1,0), box = False, align = 'left' )
...
for i in range(N):
    p_N2[i] = [2 * L2*random() - L2, 4 * L1*random() - 2*L1, 2 * L2*random() - L2] # particle is initially random positioned in container
    ra = pi*random()
    rb = 2*pi*random()
...
while True:
    ...
    for j in range(numBins):
        c.modify(j,pos = vec((((avgNinBin-positionBin[j])/N)), 2*L1 - j*seg, 0))
