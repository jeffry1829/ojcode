...
while True:
    ...
    for i in range(N-1): # the first N-1 molecules
        for j in range(i+1,N): # from i+1 to the last molecules, to avoid double checking
            if mag(COs[i].C.pos-COs[j].C.pos)<2*size:
                COs[i].C.v,COs[j].C.v=collision(COs[i].C,COs[j].C)
            if mag(COs[i].C.pos-COs[j].O.pos)<2*size:
                COs[i].C.v,COs[j].O.v=collision(COs[i].C,COs[j].O)
            if mag(COs[i].O.pos-COs[j].C.pos)<2*size:
                COs[i].O.v,COs[j].C.v=collision(COs[i].O,COs[j].C)
            if mag(COs[i].O.pos-COs[j].O.pos)<2*size:
    ...
    for CO in COs:
        ...
        total_v_P+=CO.v_P()
        total_r_K+=CO.r_K()
    c_avg_com_K.plot(pos=(t,total_com_K/times))
    c_avg_v_P.plot(pos=(t,total_v_P/times))
    c_avg_v_K.plot(pos=(t,total_v_K/times))
    c_avg_r_K.plot(pos=(t,total_r_K/times))
