...
import math
N=72
m, size = 4E-3/6E23, 0.15/50#31E-12*10
L3=0.15
...
container2 = box(length = 2*L2, height = 2*L2, width = 2*L2, opacity=0.2, color = color.yellow,
                 pos=vec(0,-L2,0))
container3 = box(length = 2*L3, height = 2*L3, width = 2*L3, opacity=0.2, color = color.yellow,
                 pos=vec(0,-2*L2-L3,0))
...
graphAxis.clear()
graphAxis.append( pos = vec(-1.5*L3, 2*L1, 0) )
graphAxis.append( pos = vec(-1.5*L3, -2*L2-2*L3, 0) )
graphLabel.pos = vec(-1.5*L3, ((2*L1+2*L2)/2) + floor.height, 0)
...
#   rate(10000)
   ...
   for i in range(N):
       ...
       if p_N2[i][1]<0 and p_N2[i][1]>=-2*L2:
           ...
           #    v_N2[i][1] = - v_N2[i][1]
       if p_N2[i][1]<-2*L2:
           if abs(p_N2[i][0]) >= L3 - size and p_N2[i][0]*v_N2[i][0] > 0:
               v_N2[i][0] = - v_N2[i][0]
           if abs(p_N2[i][2]) >= L3 - size and p_N2[i][2]*v_N2[i][2] > 0:
               v_N2[i][2] = - v_N2[i][2]
           if p_N2[i][1] >= -2*L2 - size and p_N2[i][1]<0 and v_N2[i][1] > 0 and (abs(p_N2[i][0])>L2 or abs(p_N2[i][2])>L2):
               v_N2[i][1] = - v_N2[i][1]
           if p_N2[i][1] <= -2*L2-2*L3 + size and p_N2[i][1]*v_N2[i][1] > 0:
               v_N2[i][1] = - v_N2[i][1]
   ...
   for j in range(numBins):
        c.modify(j,pos = vec(-1.5*L3 - ((positionBin[j])/N), 2*L1 - j*seg, 0))