...
main(void) {cin.tie(0);ios_base::sync_with_stdio(0);
  cin>>n>>k;sq=sqrt(n),num=k*(k+1ll)/2ll;
  ll maxx=-1e9;rep(i,1,sq+1){
    if(n%i==0 and i>=num)maxx=max(maxx,n/i);
    else if(n%i==0 and n/i>=num)maxx=max(maxx,i);
  }if(maxx==-1e9 or k>=141421){cout<<-1<<'\n';return 0;}
  rep(i,1,k)cout<<maxx*i<<' '; cout<<maxx*(n/maxx-(k*(k-1))/2)<<'\n';
  return 0;
}