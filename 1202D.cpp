...
//head
const int _n=3e5+10,MAXB=17;
int t,n;
char ans[_n];
ll C(ll x){if(x<2)return 0;return x*(x-1)/2;}
main(void) {cin.tie(0);ios_base::sync_with_stdio(0);
  cin>>t;while(t--){
    cin>>n;int tmp=0,tmpp;
    per(i,0,MAXB)if(tmp+(1<<i)<=45000 and C(tmp+(1ll<<i))<=n)tmp+=(1<<i);
    tmpp=n-C(tmp);tmp-=2;cout<<"133";rep(i,0,tmpp)cout<<'7';
    rep(i,0,tmp)cout<<'3'; cout<<"7\n";
  }
  return 0;
}