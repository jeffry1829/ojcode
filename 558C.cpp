...
//head
const int _n=1e5+10;
int n,a[_n],dis[_n],vis[_n];
main(void) {cin.tie(0);ios_base::sync_with_stdio(0);
  cin>>n;rep(i,0,n)cin>>a[i];
  rep(i,0,n){
    int now=a[i],t=32-__builtin_clz(now);
    rep(j,0,t+1)rep(k,0,17){
      if(((now>>j)<<k)<100001 and vis[(now>>j)<<k]==i)vis[(now>>j)<<k]++,dis[(now>>j)<<k]+=j+k;
    }
  }int minn=2e9;rep(i,0,100001)if(vis[i]==n)minn=min(minn,dis[i]);
  cout<<minn<<'\n';
  return 0;
}// Need to define int long long