...
main(void) {cin.tie(0);ios_base::sync_with_stdio(0);
  ...
  rep(i,1,n+1){
    ...
    rep(j,1,m+1){
      if(mp[i][j]=='.')cnt=0;
      if(mp[i][j]=='*')cnt++;
      if(cnt>2 and (cnt&1))ho[i][j-cnt+cnt/2+1]=cnt/2;
      else if(cnt>2 and cnt%2==0)ho[i][j-cnt+cnt/2+1]=cnt/2-1;
    }
  }
  rep(j,1,m+1){
    int cnt=0;
    rep(i,1,n+1){
      if(mp[i][j]=='.')cnt=0;
      if(mp[i][j]=='*')cnt++;
      if(cnt>2 and (cnt&1))ve[i-cnt+cnt/2+1][j]=cnt/2;
      else if(cnt>2 and cnt%2==0)ve[i-cnt+cnt/2+1][j]=cnt/2-1;
    }
  }
  rep(i,1,n+1)rep(j,1,m+1)if(ho[i][j] and ve[i][j])ans.pb({i,j,min(ho[i][j],ve[i][j])});
  rep(k,0,SZ(ans)){
