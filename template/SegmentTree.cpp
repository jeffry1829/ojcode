...
inline int pmod(int x, int divisor) {
  int m = x % divisor;
...
int query(int v, int l, int r, int ql, int qr) {  // 懶標記未確認運作
  if (r <= ql or l >= qr) return -1e9;
  if (ql <= l and qr >= r) return t[v];
  int m = (l + r) >> 1, res;
  push(v);
  res = max(query(2 * v + 1, l, m, ql, qr), query(2 * v + 2, m, r, ql, qr));
  pull(v);
  return res;
}
}  // namespace Seg
 ...
 public:
  vector<int> maxSlidingWindow(vector<int>& nums, int k) {
    ...
    for (int i = k - 1; i < nums.size(); i++) {
      ans.push_back(Seg::query(0, 0, n, i - k + 1, i + 1));
    }
    return ans;
  }
};