...
void bfs(int p){
  ...
  queue<PII> q;rep(i,0,n)if((a[i]&1)==p)q.push({i,0});
  while(!q.empty()){
    PII now;while(!q.empty() and vis[(now=q.front()).fi])q.pop();
    if(q.empty())return;
    vis[now.fi]=1;
    if((a[now.fi]&1)!=p)ans[now.fi]=now.se;
    rep(i,0,SZ(G[now.fi]))if(!vis[G[now.fi][i]])q.push({G[now.fi][i],now.se+1});
  }
}
main(void) {cin.tie(0);ios_base::sync_with_stdio(0);
  ...
  rep(i,0,n)cout<<(ans[i]==0?-1:ans[i])<<' '; cout<<'\n';
  return 0;
}