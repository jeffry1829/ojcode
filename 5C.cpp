...
//head
const int _n=1e6+10;
int t,n,cnt[_n],dp[_n];
char s[_n];
main(void) {cin.tie(0);ios_base::sync_with_stdio(0);
  cin>>(s+1);n=strlen(s+1);stack<int> st;
  rep(i,1,n+1){
    if(s[i]=='(')st.push(i);
    else{
      if(st.empty())continue;
      t=st.top();st.pop();
      if(dp[t-1])dp[i]=dp[t-1];
      else dp[i]=t;
    }
  }int mx=0;cnt[0]=1;
  rep(i,1,n+1)if(dp[i])cnt[i-dp[i]+1]++;
  rep(i,1,n+1)if(i-dp[i]+1>mx and dp[i])mx=i-dp[i]+1;
  cout<<mx<<' '<<cnt[mx]<<'\n';
  return 0;
}