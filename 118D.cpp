...
main(void) {
  ...
  cin>>n1>>n2>>k1>>k2;
  rep(i,0,min(n1+1,k1+1))dp[i][0][0]=1;rep(i,0,min(n2+1,k2+1))dp[0][i][1]=1;
  rep(i,1,n1+1)rep(j,1,n2+1){
    rep(k,1,min(k1+1,i+1))dp[i][j][0]+=dp[i-k][j][1],dp[i][j][0]%=mod;
    rep(k,1,min(k2+1,j+1))dp[i][j][1]+=dp[i][j-k][0],dp[i][j][1]%=mod;
  }
