...
main(void) {cin.tie(0);ios_base::sync_with_stdio(0);
  cin>>n>>s>>t;do{
    string tmp="";rep(i,0,n)tmp+=abc; res.pb(tmp);
    tmp="";tmp=string(n,abc[0])+string(n,abc[1])+string(n,abc[2]);
    res.pb(tmp);
  }while(next_permutation(all(abc)));
  rep(i,0,SZ(res))rep(j,0,(int)res[i].length()-1){
    if(res[i][j]==s[0] and res[i][j+1]==s[1]) break;
    if(res[i][j]==t[0] and res[i][j+1]==t[1]) break;
    if(j==(int)res[i].length()-2){cout<<"YES\n"<<res[i]<<'\n';return 0;}
  }cout<<"NO\n";
  return 0;
}