...
main(void) {
  ...
  cin >> t;while(t--){
    cin>>n>>m;rep(i,0,m)cin>>s[i].fi>>s[i].se;sort(s,s+m);
    suf[m-1]=s[m-1].fi;per(i,0,m-1)suf[i]=suf[i+1]+s[i].fi;suf[m]=0;
    ll maxx=suf[max(m-n,0)];
    rep(i,0,m){
      int nn=n;
      ll tmp=0;int cnt=upper_bound(s,s+m,mp(s[i].se,-1))-s;cnt=max(cnt,max(m-n,0));
      tmp+=suf[cnt];nn-=(m-cnt);
      if(s[i].fi<=s[i].se)nn--,tmp+=s[i].fi;
      tmp+=1ll*nn*s[i].se;
      maxx=max(maxx,tmp);
    }
    cout<<maxx<<'\n';
