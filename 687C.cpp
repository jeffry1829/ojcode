...
main(void) {cin.tie(0);ios_base::sync_with_stdio(0);
  cin>>n>>k;rep(i,1,n+1)cin>>c[i];
  dp[0][0][0]=1;
  rep(i,1,n+1){
    dp[i&1][0][0]=1;
    rep(j,1,k+1){
      rep(u,0,k+1)dp[i&1][j][u]|=dp[!(i&1)][j][u];
      if(j<c[i])continue;
      rep(u,0,k+1)dp[i&1][j][u]|=dp[!(i&1)][j-c[i]][u];
      rep(u,0,k+1)if(u>=c[i])dp[i&1][j][u]|=dp[!(i&1)][j-c[i]][u-c[i]];
    }
  }rep(i,0,k+1)if(dp[n&1][k][i])ans.pb(i); cout<<SZ(ans)<<'\n';
